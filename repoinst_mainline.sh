...
finalize_yum_repo ()
{
  ...
  yum install -y pygpgme --disablerepo='nginx-alpn_mainline'
  ...
  if [ "$?" != "0" ]; then
    ...
    sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/nginx-alpn_mainline.repo
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='nginx-alpn_mainline'
  ...
  echo "Generating yum cache for nginx-alpn_mainline..."
  yum -q makecache -y --disablerepo='*' --enablerepo='nginx-alpn_mainline'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh nginx-alpn_mainline
}
...
main ()
{
  ...
  yum_repo_config_url="https://packagecloud.io/install/repositories/nginx-alpn/mainline/config_file.repo?os=${os}&dist=${dist}&source=script"

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/nginx-alpn_mainline.repo
  else
    yum_repo_path=/etc/yum.repos.d/nginx-alpn_mainline.repo
  fi
